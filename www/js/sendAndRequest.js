...
app.controller('incomingRequestsCtrl', function($scope, $rootScope, $firebaseArray, $http, $validateLogin, requestCount, $ionicPopup, ionicToast) {
    ...
    $scope.reject = function(request) { reject(request, $rootScope, $http, ionicToast) }
    ...
    firebaseCollection.$loaded().then(function() {
        ...
        if (firebaseCollection.length > 0) {
            $scope.loaded = true;
...
function reject(request, $rootScope, $http, ionicToast) {
    request.wait = true;
    request.approvedBy = $rootScope.currentUser.userId;
    var payload = makePayload(request);
    $http.post(addressHost + "broadcast/request/reject", { payload: payload })
        .then(function(res) {
            request.wait = false;
            ionicToast.show('Rejected!', 'bottom', false, 2500);
        })
        .catch(function(err) {
            request.wait = false;
            console.log(header, err);
            ionicToast.show('Something Went Wrong!', 'bottom', false, 2500);
        });
